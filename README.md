...
In general, the functions and methods inside `digifi` propagate errors so that the users can decide on the error handling techniques applicable to their use case. However, for some numerical solutions closures are used which can panic at runtime.
...
1. Golden Ratio, Nelder-Mead and L-BFGS Numerical Solvers (<https://github.com/to266/optimize>): Unfortunately, the crate `optimize` is outdated, but it contains really good implementations of different numerical solvers. `optimize` is not a dependency to this library, but we have used extracts from its source code and slightly changed it to optimise it for our needs. We also reused some parts of their documentation as it was very well explained by its developers. Nonetheless, `optimize` is a great package and we hope there will be a renewed support for it.
2. 2. Gradient Descent and Line Search Algorithms (<https://github.com/aomader/optimization-rust>): We have adapted the gradient descent and line search algorithms from the crate `optimization` for the same reasons as above. We appreciate the great code base of the package, but unfortunately it is not actively maintained, so we decided to adapt the code from it as aopposed to use it as our dependency.
...
1. Algorithms for ln Gamma, ln Beta, Regularized Incomplete Beta and Inverse Regularized Incomplete Beta functions (<https://github.com/statrs-dev/statrs>): The implementation of these functions were taken from `statrs`.
